...
def write_csv(data, filepath='./dags/Current/'):

    filename = 'Mnemonics_' +  datetime . now (). strftime ( '%Y_%m_%d_%H_%M_%S' ) + '.csv'
...
def  mnemonic_scraper_all ( page_quantity ):
    ...
    for  page  in  range ( 0 , page_quantity ):
      final_data  +=  mnemonic_scraping ( page )

    write_csv ( final_data )

def mnemonic_scraping ( page_number ):
    ind_dict = {}
    final_data = []
